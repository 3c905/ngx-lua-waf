...
# -*- coding: utf-8 -*-
"""
分析 www.alcon.cn.log，根据当前 WAF 规则判断哪些恶意/异常请求未被拦截。
日志格式（第一字段为占位符/上游地址，第二字段为真实客户端 IP）：
- <upstream> <client_ip> - - [time] "request" status size "referer" "UA"
"""
...
LOG_FILE = r"D:\lumin\desktop\www.alcon.cn.log"
WAFCF_DIR = "wafconf"
...
def parse_log_line(line):
    ...
    if not m:
        return None
    _, ip, time, request, status, size, referer, ua = m.groups()
...
def main():
    ...
    for req in scan_success[:50]:
        print(f"  [{req['status']}] {req['ip']} {req['method']} {req['uri'][:100]}")
        print(f"       UA: {req['ua'][:80]}")
    print(f"   合计 {len(scan_success)} 条\n")

    print("=" * 80)
    print("六、404 路径 TOP 20（未命中 WAF 规则）")
    print("=" * 80)
    for path, count in path_404_counter.most_common(20):
        print(f"  {count:5d}  {path[:120]}")
    print()

    print("=" * 80)
    print("七、请求量 TOP 20 的 IP")
    print("=" * 80)
